...
*   **Документооборот (Workflow):** Поддержка черновиков (`is_draft`), рабочих смен и электронных подписей. Утверждение анкеты блокирует её от дальнейших изменений.
...
| `POST`   | `/api/v1/templates/{id}/clone/`   | Клонирует существующий шаблон                              |
...
### 📝 Домен Вложений (Attachments)

| Метод    | URL                                         | Описание                                              |
|:---------|:--------------------------------------------|:------------------------------------------------------|
| `POST`   | `/api/v1/attachments/`                      | Прикрепить файл/фото к анкете (`multipart/form-data`) |
| `GET`    | `/api/v1/attachments/{id}/`                 | Получить прикрепленный к анкете файл                  |
| `DELETE` | `/api/v1/attachments/{id}/`                 | Удалить вложение у анкеты                             |
| `GET`    | `/api/v1/attachments/{id}/download/`        | Загрузить с сервера файл, прикрепленный к анкете      |
---
