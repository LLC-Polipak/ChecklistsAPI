...
class ChecklistExcelExporter:
    ...
    def export(cls, result: ChecklistResult) -> bytes:
        """
        Сформировать Excel-документ на основе заполненной анкеты чек-листа.

        Алгоритм работы:
        1. Сформировать "шапку" с метаданными.
        2. Отрисовать таблицу ответов с группировкой.
        3. Заменить пустые значения на визуальные прочерки.
        4. Вывести блок истории подписаний.

        Args:
            result: Объект заполненной анкеты из базы данных.

        Returns:
            bytes: Бинарная последовательность файла для отправки клиенту.
        """
    ...
    def _format_empty(value):
        """Заменить пустые значения и None на прочерк."""
